    ...
    match state.group_manager.create_group(group.clone()) {
        Ok(_) => {
            ...
            match state.group_manager.get_group(&group_key) {
                Some(created_group) => {
                    (StatusCode::CREATED, Json(ApiResponse::success(created_group)))
                }
                None => (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    Json(ApiResponse::<ServiceGroup>::error(
                        "Failed to retrieve created group".to_string(),
                    )),
                ),
            }
        }
        ...
        Some(mut group) => {
            if let Some(mut tags) = group.tags {
                ...
                match state.group_manager.update_group(group.clone()) {
                    Ok(_) => (StatusCode::OK, Json(ApiResponse::success(group))),
                    Err(e) => {
                        (StatusCode::BAD_REQUEST, Json(ApiResponse::<ServiceGroup>::error(e)))
                    }
                }
            } else {
                (
...
pub async fn get_group_instances(
    ...
    match state.group_manager.get_group(&group_key) {
        Some(group) => {
            let instances = state.registry_service.get_instances_by_group(
                &group.service_id,
                &group.name,
        ...
        None => (
            StatusCode::NOT_FOUND,
            Json(ApiResponse::<Vec<artemis_common::model::Instance>>::error(format!(
                "Group {} not found",
                group_key
            ))),
        ),
    ...
    match state.route_manager.create_rule(rule.clone()) {
        Ok(_) => {
            ...
            match state.route_manager.get_rule(&rule.route_id) {
                Some(created_rule) => {
                    (StatusCode::CREATED, Json(ApiResponse::success(created_rule)))
                }
                None => (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    Json(ApiResponse::<RouteRule>::error(
                        "Failed to retrieve created rule".to_string(),
                    )),
                ),
            }
        }
...
pub async fn update_rule_group(
    ...
    match group {
        ...
        None => (
            StatusCode::NOT_FOUND,
            Json(ApiResponse::<RouteRuleGroup>::error(format!(
                "Group {} not found in rule {}",
                group_id, rule_id
            ))),
        ),
    }
...
pub async fn batch_add_service_instances(
    ...
    // 批量添加
    match state.group_manager.batch_add_service_instances(group_id, req.instances).await {
        Ok(count) => (StatusCode::CREATED, Json(ApiResponse::success(count))),
        Err(e) => (StatusCode::BAD_REQUEST, Json(ApiResponse::<usize>::error(e))),
    }
}
...
mod tests {
    ...
    fn test_list_groups_query_by_service() {
        let query = ListGroupsQuery { service_id: Some("service1".to_string()), region_id: None };
    ...
    fn test_list_groups_query_by_region() {
        let query = ListGroupsQuery { service_id: None, region_id: Some("us-east".to_string()) };
    ...
    fn test_update_group_request_partial() {
        let req = UpdateGroupRequest { description: Some("updated".to_string()), status: None };
    ...
    fn test_update_rule_group_request() {
        let req = UpdateRuleGroupRequest { weight: 75 };
    ...
    fn test_add_group_tags_request() {
        let tags = vec![
            GroupTag { key: "env".to_string(), value: "prod".to_string() },
            GroupTag { key: "team".to_string(), value: "platform".to_string() },
        ];
        let req = AddGroupTagsRequest { tags: tags.clone() };
    ...
    fn test_batch_add_service_instances_request() {
        ...
        let req = BatchAddServiceInstancesRequest { instances: instances.clone() };
    ...
    fn test_batch_add_service_instances_request_empty() {
        let req = BatchAddServiceInstancesRequest { instances: vec![] };
    ...
    fn test_update_group_request_empty() {
        let req = UpdateGroupRequest { description: None, status: None };
        assert!(req.description.is_none());
        assert!(req.status.is_none());
    }
    ...
    fn test_update_group_request_description_only() {
        let req =
            UpdateGroupRequest { description: Some("new description".to_string()), status: None };
    ...
    fn test_update_group_request_status_only() {
        let req = UpdateGroupRequest { description: None, status: Some(GroupStatus::Active) };
        assert!(req.description.is_none());
    ...
    fn test_update_rule_request_empty() {
        let req = UpdateRuleRequest { name: None, description: None, strategy: None };
    ...
    fn test_list_groups_query_empty() {
        let query = ListGroupsQuery { service_id: None, region_id: None };
        assert!(query.service_id.is_none());
        assert!(query.region_id.is_none());
    }
    ...
    fn test_get_group_instances_query_empty() {
        let query = GetGroupInstancesQuery { region_id: None, zone_id: None };
        assert!(query.region_id.is_none());
    ...
    fn test_get_group_instances_query_region_only() {
        let query =
            GetGroupInstancesQuery { region_id: Some("us-west".to_string()), zone_id: None };
    ...
    fn test_get_group_instances_query_zone_only() {
        let query = GetGroupInstancesQuery { region_id: None, zone_id: Some("zone2".to_string()) };
        assert!(query.region_id.is_none());
    ...
    fn test_add_group_tags_request_empty() {
        let req = AddGroupTagsRequest { tags: vec![] };
    ...
    fn test_add_group_tags_request_single() {
        let req = AddGroupTagsRequest {
            tags: vec![GroupTag { key: "version".to_string(), value: "1.0.0".to_string() }],
        };
