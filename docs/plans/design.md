...
3. [核心数据模型（artemis-common）](#3-核心数据模型artemis-common)
4. [业务逻辑层（artemis-service）](#4-业务逻辑层artemis-service)
5. [Web层（artemis-server）](#5-web层artemis-server)
...
#### 1. artemis-common (lib)
- **职责**: 核心类型、trait定义、错误类型
- **依赖**: 最小化（serde、chrono、thiserror）
- **导出**: 所有模型和trait

#### 2. artemis-service (lib)
- **职责**: 业务逻辑实现
- **依赖**: artemis-common + tokio + dashmap + governor
- **模块**: registry、discovery、lease、cache、cluster、replication

#### 3. artemis-server (lib)
- **职责**: HTTP/WebSocket API层
- **依赖**: artemis-common + artemis-service + axum + tower
- **导出**: API路由和WebSocket处理器

#### 4. artemis-management (lib)
- **职责**: 管理功能和持久化
- **依赖**: artemis-common + artemis-service + sqlx
- **模块**: instance、group、route、dao

#### 5. artemis-client (lib)
- **职责**: 客户端SDK（独立发布）
- **依赖**: artemis-common + reqwest + tokio-tungstenite
- **注意**: 独立于bin，供外部用户使用

#### 6. artemis (bin only)
- **职责**: 唯一可执行程序
- **依赖**: artemis-common + artemis-service + artemis-server + artemis-management + clap
