# Artemis 管理功能集成测试

## 概述

`test-management.sh` 是一个全面的 Artemis 管理功能集成测试脚本,用于验证所有管理 API 的正确性。

## 测试覆盖

该脚本测试 **6 大管理模块**,共 **29 个测试用例**:

### 📦 模块列表

| 模块 | 测试数 | 功能描述 |
|------|--------|----------|
| **实例管理** (Instance Management) | 5 | 单实例拉入/拉出、状态查询、操作历史 |
| **服务器管理** (Server Management) | 3 | 服务器级别批量拉入/拉出 |
| **Zone管理** (Zone Management) | 4 | Zone级别批量拉入/拉出、状态管理 |
| **金丝雀发布** (Canary Deployment) | 5 | 金丝雀配置、IP白名单、启用/禁用 |
| **分组路由** (Group Routing) | 8 | 分组创建、路由规则、加权策略 |
| **审计日志** (Audit Logs) | 3 | 操作日志查询 |

## 快速开始

### 前置条件

1. **启动 Artemis 集群**:
   ```bash
   ./scripts/cluster.sh start 3
   ```

2. **确保服务健康**:
   ```bash
   curl http://localhost:8080/health
   ```

### 运行测试

#### 1. 测试所有模块 (推荐)
```bash
./scripts/test-management.sh all
```

#### 2. 测试单个模块
```bash
# 实例管理
./scripts/test-management.sh instance

# 服务器管理
./scripts/test-management.sh server

# Zone管理
./scripts/test-management.sh zone

# 金丝雀发布
./scripts/test-management.sh canary

# 分组路由
./scripts/test-management.sh routing

# 审计日志
./scripts/test-management.sh audit
```

## 测试结果示例

### 成功输出
```
=========================================
测试总结
=========================================
总测试数: 29
通过: 29
失败: 0

✓ 所有测试通过! 🎉
```

### 模块测试输出
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  模块: 实例管理 (Instance Management)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

[测试 2] 拉出单个实例 (Pull Out)
✓ 通过

[测试 3] 查询实例是否被拉出
✓ 通过

[测试 4] 查询实例操作历史
✓ 通过

[测试 5] 拉入实例 (Pull In)
✓ 通过

[测试 6] 验证实例已拉入
✓ 通过
```

## 测试详情

### 实例管理 (Instance Management)

```bash
# 测试流程
1. 拉出单个实例 (Pull Out)
2. 查询实例是否被拉出
3. 查询实例操作历史
4. 拉入实例 (Pull In)
5. 验证实例已恢复
```

**API 端点**:
- `POST /api/management/instance/operate-instance.json`
- `POST /api/management/instance/is-instance-down.json`
- `POST /api/management/instance/get-instance-operations.json`

### 服务器管理 (Server Management)

```bash
# 测试流程
1. 拉出整个服务器 (所有实例)
2. 查询服务器状态
3. 拉入服务器 (恢复所有实例)
```

**API 端点**:
- `POST /api/management/server/operate-server.json`
- `POST /api/management/server/is-server-down.json`

### Zone管理 (Zone Management)

```bash
# 测试流程
1. 拉出整个Zone (所有实例)
2. 查询Zone状态
3. 查询所有Zone操作记录
4. 拉入Zone (恢复所有实例)
```

**API 端点**:
- `POST /api/management/zone/pull-out`
- `POST /api/management/zone/pull-in`
- `GET /api/management/zone/status/:zone_id/:region_id`
- `GET /api/management/zone/operations`

### 金丝雀发布 (Canary Deployment)

```bash
# 测试流程
1. 创建金丝雀配置 (IP白名单)
2. 查询金丝雀配置
3. 启用金丝雀发布
4. 查询所有金丝雀配置
5. 删除金丝雀配置
```

**API 端点**:
- `POST /api/management/canary/config`
- `GET /api/management/canary/config/:service_id`
- `POST /api/management/canary/enable`
- `GET /api/management/canary/configs`
- `DELETE /api/management/canary/config/:service_id`

### 分组路由 (Group Routing)

```bash
# 测试流程
1. 创建金丝雀分组
2. 创建稳定分组
3. 创建加权路由规则 (canary:10%, stable:90%)
4. 添加金丝雀分组到规则 (权重10)
5. 添加稳定分组到规则 (权重90)
6. 发布路由规则
7. 查询规则详情
8. 停用路由规则
```

**API 端点**:
- `POST /api/routing/groups`
- `POST /api/routing/rules`
- `POST /api/routing/rules/:rule_id/groups`
- `POST /api/routing/rules/:rule_id/publish`
- `POST /api/routing/rules/:rule_id/unpublish`
- `GET /api/routing/rules/:rule_id`

### 审计日志 (Audit Logs)

```bash
# 测试流程
1. 查询所有审计日志
2. 查询实例操作日志
3. 查询服务器操作日志
```

**API 端点**:
- `GET /api/management/audit/logs`
- `GET /api/management/audit/instance-logs`
- `GET /api/management/audit/server-logs`

## 故障排查

### 常见问题

#### 1. 服务器未启动
```
错误: 服务器启动超时
解决: ./scripts/cluster.sh start 3
```

#### 2. 端口冲突
```
错误: 端口 8080 已被占用
解决: ./scripts/cluster.sh stop && ./scripts/cluster.sh start 3
```

#### 3. 测试数据冲突
```
错误: Group already exists
解决: 重启集群清空内存数据
      ./scripts/cluster.sh stop && ./scripts/cluster.sh start 3
```

## 环境变量

脚本使用以下环境变量(可自定义):

```bash
BASE_URL="http://localhost:8080"      # Artemis 服务器地址
SERVICE_ID="test-mgmt-service"        # 测试服务ID
REGION_ID="us-east"                   # 测试Region
ZONE_ID="zone-1"                      # 主Zone
ZONE_ID_2="zone-2"                    # 辅助Zone
```

## 测试数据

脚本会自动:
1. **准备阶段**: 注册 5 个测试实例 (zone-1: 3个, zone-2: 2个)
2. **测试阶段**: 对实例执行各种管理操作
3. **清理阶段**: 删除所有测试数据 (分组、规则、配置)

## 输出格式

测试脚本使用颜色标记:
- 🟢 **绿色**: 测试通过
- 🔴 **红色**: 测试失败
- 🟡 **黄色**: 测试步骤
- 🔵 **蓝色**: 模块标题
- 🔷 **青色**: 章节标题

## 退出码

- `0`: 所有测试通过
- `1`: 有测试失败或服务器启动失败

## 注意事项

1. **集群模式**: 脚本默认连接到 `localhost:8080`,适用于集群环境
2. **数据隔离**: 使用独立的测试服务ID,不会影响生产数据
3. **自动清理**: 测试结束后自动清理所有测试数据
4. **幂等性**: 可以重复运行,每次都会清理旧数据

## 贡献

如需添加新的测试用例:
1. 在对应模块函数中添加测试步骤
2. 更新 `TOTAL_TESTS` 计数
3. 在 `cleanup_test_data()` 中添加清理逻辑

## 许可证

MIT OR Apache-2.0
